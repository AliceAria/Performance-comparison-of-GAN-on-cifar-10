...
class VAE(object):
    ...
    def encoder(self, x, is_training=True, reuse=False):
        ...
        with tf.variable_scope("encoder", reuse=reuse):
            ...
            net = lrelu(conv2d(net, 128, 4, 4, 2, 2, name='en_conv2'))
            net = lrelu(conv2d(net, 256, 4, 4, 2, 2, name='en_conv3'))
            net = tf.reshape(net, [self.batch_size, -1])
            net = lrelu(linear(net, 1024, scope='en_fc4'))
            gaussian_params = linear(net, 2 * self.z_dim, scope='en_fc5')
    ...
    def decoder(self, z, is_training=True, reuse=False):
        ...
        with tf.variable_scope("decoder", reuse=reuse):
            net = tf.nn.relu(linear(z, 1024, scope='de_fc1'))
            net = tf.nn.relu(linear(net, 256 * 4 * 4, scope='de_fc2'))
    ...
    def build_model(self):
        ...
        with tf.control_dependencies(tf.get_collection(tf.GraphKeys.UPDATE_OPS)):
            self.optim = tf.train.AdamOptimizer(self.learning_rate_D, beta1=self.beta1) \
