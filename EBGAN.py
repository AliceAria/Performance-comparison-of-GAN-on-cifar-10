...
class EBGAN(object):
    ...
    def discriminator(self, x, is_training=True, reuse=False):
        ...
        with tf.variable_scope("discriminator", reuse=reuse):
            ...
            code = tf.nn.relu(linear(net, 32, scope='d_fc6')) # bn and relu are excluded since code is used in pullaway_loss
            net = tf.nn.relu(bn(linear(code, 64 * 16 * 16, scope='d_fc3'), is_training=is_training, scope='d_bn3'))
            net = tf.reshape(net, [self.batch_size, 16, 16, 64])
            out = tf.nn.sigmoid(deconv2d(net, [self.batch_size, 32, 32, 3], 4, 4, 2, 2, name='d_dc5'))
    ...
    def build_model(self):
        ...
        with tf.control_dependencies(tf.get_collection(tf.GraphKeys.UPDATE_OPS)):
            ...
            self.g_optim = tf.train.AdamOptimizer(self.learning_rate_D, beta1=self.beta1) \
